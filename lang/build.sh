...
do
    ...
    sed -e "s/<%= id %>/$LANGUAGE/g" -e "s/<%= version1 %>/$VERSION1/g" -e "s/<%= version2 %>/$VERSION2/g" -e "s/<%= version3 %>/$VERSION3/g" ./template.tpl | awk -v v="$JOINED" '{gsub("<%= data %>",v); print}' | sed -e "s/'/\\\'/g" -e "s/\"/'/g" | /bin/js-beautify -j -> ./$LANGUAGE.js
done
